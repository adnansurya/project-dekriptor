...
def write_log_json(proses, file_asal, file_tujuan, status, durasi="", error="",
                   ...
                   avg_power=None, cpu_awal=None, ram_awal=None):
    log_entry = {
...
def monitor_power():
    ...
    while monitoring:
        ...
        if data:
            ...
            power_info.append(avg)
        time.sleep(1)

def get_average_power_usage():
    if power_info:
        return sum(power_info) / len(power_info)
    return None
...
stat_label.pack(pady=3)
power_label = tk.Label(root, text="Daya: - W", fg="gray")
power_label.pack(pady=3)
...
def update_stat_label():
    ...
    if power_data:
        avg_power = sum(power_data.values()) / len(power_data)
        power_label.config(text=f"Daya: {avg_power:.2f} W")
    else:
        power_label.config(text="Daya: - W")
